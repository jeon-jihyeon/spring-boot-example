...
class TeamCommandJoinPlayerServiceTest {
    ...
    @InjectMocks
    private TeamCommandJoinPlayerService service;

    @Test
    void shouldAddPlayer() {
        when(commandRepository.findById(TEAM_ID)).thenReturn(TEAM_DATA);
        when(commandRepository.save(any())).thenReturn(TEAM_DATA);

        final TeamData data = service.invoke(new TeamJoinPlayerCommand(PLAYER_ID, TEAM_ID));
