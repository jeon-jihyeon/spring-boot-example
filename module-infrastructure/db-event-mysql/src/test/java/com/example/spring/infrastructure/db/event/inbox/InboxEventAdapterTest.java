...
class InboxEventAdapterTest extends BaseContextTest {
    ...
    void shouldBeSavedAndFound() {
        ...
        adapter.save(new DomainEvent(1L, DomainEventState.CREATED, DomainEventType.CREATE, "model", 2L, now, now, now));

        final InboxEventEntity found = repository.findById(1L).orElseThrow(RuntimeException::new);
        assertThat(found.getId()).isEqualTo(1L);
        assertThat(found.getState()).isEqualTo(DomainEventState.CREATED);
        ...
        assertThat(found.getProcessedAt()).isEqualTo(now);
